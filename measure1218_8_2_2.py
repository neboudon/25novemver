...
ANGLE_LEFT_DEG = 45
ANGLE_RIGHT_DEG = 135
THRESHOLD_SIDE = 5.0   # 左右レーンのしきい値(cm)
THRESHOLD_CENT = 13.0  # 中央レーンのしきい値(cm)
PERCENTILE_THRESHOLD = 80  # 上位パーセンタイル
    ...
    while True:
        ...
        for lane_name, mask in masks.items():
            ...
            if target_heights.size > 0:
                ...
                # 2. 指定のしきい値以上の領域を赤枠で囲む
                thresh_p = np.percentile(target_heights, PERCENTILE_THRESHOLD)
                # 判定用マスク：現在のレーン内 かつ 高さが上位%の境界以上 かつ 指定しきい値以上
                obs_limit = max(thresh_p, current_threshold)
                ...
                for cnt in contours:
                    if cv2.contourArea(cnt) < 50: continue # ノイズ除外
        ...
        cv2.imshow('Top % Detection & Distance', color_image)
