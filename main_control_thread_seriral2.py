...
# ★★★ 壁検出スレッド(wall_thread_func)のインポートを削除 ★★★
from robot_vision_thread_headless1 import steering_thread_func, gravity_thread_func
# ↑↑↑ ファイル名変更を推奨 ↑↑↑

### --- ステートマシンの状態定義 ---
# ★★★ 壁検出を使わないため、ステートマシン関連をコメントアウト ★★★
# STATE_DRIVING = 0
# STATE_STOPPED = 1
...
# CAMERA_INDEX_WALL = 1     # ★★★ 壁検出用カメラの番号 (コメントアウト) ★★★
...
def main():
    ...
    print("[メイン]: 操舵スレッドを起動します...") # ★★★ メッセージ変更 ★★★
    t_steering.start()
    # t_wall.start() # ★★★ 壁検出スレッドの起動をコメントアウト ★★★
    ...
    try:
        while True:
            ...
            with lock:
                ...
                # is_wall_detected = (shared_state['wall_detected'] == 1) # ★★★ 壁検出フラグの読み出しを削除 ★★★
                is_wall_detected = False # ★★★ 常に壁なしとして扱う (デバッグ表示用) ★★★
            ...
            final_command = steering_command # ★★★ ステートマシンを削除し、常に操舵コマンドを最終コマンドとする ★★★

            # ★★★ 壁検出とステートマシンによる停止ロジックをすべてコメントアウト ★★★
            ...
            # --- 4-5. 状態の表示 (標準出力) ---
            # state_text = "DRIVING" if current_state == STATE_DRIVING else "STOPPED" # ★★★ 削除 ★★★
            mode_text = f"Mode: {STEERING_MODE}"
            # ★★★ 状態(State)の表示を削除 ★★★
            print(f"状態: DRIVING, {mode_text}, " # ★★★ 状態をDRIVINGに固定 ★★★
        ...
        t_steering.join()
        # t_wall.join() # ★★★ 壁検出スレッドの終了待ちをコメントアウト ★★★
