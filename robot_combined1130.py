...
from function1130 import realsense_capture_thread, vision_processing_thread, optical_flow_water_detection, wall_control_thread, command_sending
...
def main():
    #共有変数の初期化
    shared_state ={
        'latest_frame': None,    # 最新のRGB画像 (Numpy)
        'latest_depth': None,    # Depth画像 (Numpy) ★追加
        'depth_scale': 0.001,    # 深度スケール ★追加
        'prev_frame': None,      # 一つ前のRGB
        'new_frame_flag': False, #新しい画像の取得を確認　#ここまでは使用を確認
        'steering_value': 0.0, #現在の操舵値（重心or消失点）
        'steering_success': False, #消失点検出成功フラグ
        'water_detected': False, #これ多分水検出フラグ
        'wall_side': None, 
        'stop': False, #これなんだろ？
        'stop_wall_control': False, #壁追従スレッド停止フラグ
        'mode': 'DRIVING', #これの意味を確認する
        #'wall_command': None #壁追従用コマンド
    }
    ...
    try:
        while True:
            ...
            with lock:
                ...
                current_mode = shared_state['mode'] #現在のモード
                is_water_detected = shared_state['water_detected'] #水検出フラグ 
                detected_wall_side = shared_state['wall_side'] #検出された壁の側面 ('left' or 'right')
            ...
            if is_water_detected and current_mode == 'DRIVING':
                ...
                # 壁制御スレッドを起動
                t_wall_control = threading.Thread(target=wall_control_thread, args=(shared_state, lock, ser, detected_wall_side,serial_lock))
                t_wall_control.start()
                
            elif current_mode == 'WALL_FOLLOWING' and not is_water_detected:#水が消えたらDRIVINGに戻る
                ...
                if t_wall_control is not None: #壁制御スレッドの終了を待機
            ...
            if current_mode == 'DRIVING':
                ...
                command_sending(ser, command_to_send, serial_lock)
