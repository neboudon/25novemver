...
def main():
    ...
    try:
        while True:
            ...
            # -------------------------------------------------------
            detected_type = None
            detected_start = None # 障害物の始まり（足元側）
            detected_end = None   # 障害物の終わり（頭側）
            ...
            REQ_PIXELS = 15  # 何ピクセル続いたら「乗り越え不可」とみなすか（調整箇所）
            WALL_THRESH_RATIO = 0.3  # 勾配が基準の30%以下なら「壁」
            CLIFF_THRESH_RATIO = 3.0 # 勾配が基準の3倍以上なら「崖」
            ...
            for y in range(scan_start, scan_end, -1):
                ...
                is_wall_cond = (current_diff < ground_slope * WALL_THRESH_RATIO) and (val > 0.5)
                
                #崖 (Cliff): 勾配が急激に落ちる、またはデータが消失(奥に落ち込む)
                is_cliff_cond = (current_diff > ground_slope * CLIFF_THRESH_RATIO) or (prev_val > 0.5 and val < 0.1)
                
                if is_wall_cond:
                    wall_pixel_count += 1
                    cliff_pixel_count = 0 # 状態が切り替わったらリセット
                elif is_cliff_cond:
                    cliff_pixel_count += 1
                    wall_pixel_count = 0
                else:
                    ...
                    cliff_pixel_count = 0
                # --- 確定判定 ---
                # 最も手前の障害物を優先してループを抜ける
                if wall_pixel_count > REQ_PIXELS:
                    detected_type = "WALL"
                    detected_start = y + wall_pixel_count
                    detected_end = y
                    break
                
                if cliff_pixel_count > REQ_PIXELS:
                    detected_type = "CLIFF"
                    detected_start = y + cliff_pixel_count
                    detected_end = y
                    break
            ...
            if detected_type is not None:
                # 色設定: 壁=赤(0,0,255), 崖=黄(0,255,255)
                box_color = (0, 0, 255) if detected_type == "WALL" else (0, 255, 255)
                
                # バウンディングボックス
                # start(下側) から end(上側) までを囲む
                cv2.rectangle(color_img, (x_start, detected_end), (x_end, detected_start), box_color, 2)
                
                # 文字表示 (WALL or CLIFF)
                label = f"{detected_type} DETECTED"
                cv2.putText(color_img, label, (x_start - 80, detected_start), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, box_color, 2)
                
                # 塗りつぶし（半透明風）
                overlay = color_img.copy()
                cv2.rectangle(overlay, (x_start, detected_end), (x_end, detected_start), box_color, -1)
            ...
            # グラフ描画関数へ渡す (変数名を新しいものに合わせる)
            # draw_analysis_graph関数の引数 obstacle_detected_start/end に、今回の結果を渡す
            obstacle_detected_start = detected_start
            obstacle_detected_end = detected_end
